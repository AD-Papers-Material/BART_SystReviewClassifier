...
This is the general query generation mechanism:
...
# time. By default results will be saved in file called rule_data.rds in the
# session folder.
...
It is advisable then to consider removing the less specific rules iteratively from the new query until a manageable number of records is obtained, considering that the probability of finding new positive matches not already found by more specific queries drops exponentially.\
...
It may be possible that during the first CR iteration, the number of records requiring manual review is a significant number:
...
remember to run `consolidate_results(session_name)` if you want these results to take into account the manual review of the predicted labels.\
...
with `records` being the fully annotated data frame or a file path to it.
...
Also, the function creates a plot to visualise the individual impact on performance of each hyperparameter.
