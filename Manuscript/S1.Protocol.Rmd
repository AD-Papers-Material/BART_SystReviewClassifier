...
Add intallation and setup info here
...
The function also creates or update a **journal** (a CSV or Microsoft Excel file) with all the information about sessions, queries and search results.\
Some services (IEEE, WOS) requires API key to access the data; an API key is not mandatory for Pubmed but can help avoid quota limitations in case of a large number of results.
IEEE can also be searched without API;
the framework will utilize a webscraping engine in case the API key is missing, but the success of this method is not guaranteed.
See Supplemental Material S3 for detailed instruction on preliminary steps and tips for each supported search engine.
These keys can be stored in the `secrets.R` file with the following example code (the "*baysren*" prefix identify the package specific options):
